...
T35Prefix::T35Prefix(const std::string& input) {
    parse(input);
}
...
std::vector<uint8_t> T35Prefix::toBytes() const {
    return hexToBytes(hexString);
}

bool T35Prefix::isValid() const {
    return !hexString.empty() && isValidHex(hexString);
}
...
bool T35Prefix::isValidDescription(const std::string& desc) {
    ...
    return std::none_of(desc.begin(), desc.end(), [](unsigned char c) {
        return c == ':' || c <= 0x1F;
    });
}
