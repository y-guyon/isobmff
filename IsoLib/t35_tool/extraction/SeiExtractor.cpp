...
MP4Err SeiExtractor::extract(const ExtractionConfig& config, MetadataMap* outItems) {
    ...
    for (u32 sampleNum = 1; sampleNum <= videoSampleCount; sampleNum++) {
        ...
        err = MP4GetIndMediaSample(videoMedia, sampleNum, videoSampleH, &videoSize,
                                    &videoDTS, &videoCTSOffset, &videoDuration,
                                    &videoFlags, &videoDescIndex);
        if (err != MP4NoErr) {
        ...
        if (metadataRemain > 0 && currentMetadata) {
            std::vector<uint8_t> sei = buildSeiNalu(
                currentMetadata->payload.data(),
                static_cast<uint32_t>(currentMetadata->payload.size()),
                config.t35Prefix
            );
