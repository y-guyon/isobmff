...
static std::string sampleFlagsToStr(u32 flags) {
    ...
    if (flags & MP4MediaSampleNotSync)
        labels.push_back("non-sync");
    else
        labels.push_back("sync");

    if (flags & MP4MediaSampleHasCTSOffset)
        labels.push_back("CTS-offset");
...
{
  std::cout << "    Sample#" << sampleIndex + 1
          << " size=" << sampleSize
          << " flags=" << sampleFlagsToStr(flags)
          << " CTS=" << cts
          << " DTS=" << dts << "\n";
  ...
  while (off < sampleSize) {
    ...
    if (off + total > sampleSize) {
      std::cout << "      !! Truncated OBU at +" << off
                << " need=" << total << " have=" << (sampleSize - off) << "\n";
      break;
    }
    std::cout << "      OBU[" << idx++ << "] @+" << off
              << " type=" << h.obu_type << " (" << obuTypeName(h.obu_type) << ")"
              << " spatial_id=" << h.spatial_id
...
int main(int argc, char** argv) {
  ...
  for (u32 trackNumber = 1; trackNumber <= trackCount; ++trackNumber) {
    ...
    if (sampleEntryType == MP4_FOUR_CHAR_CODE('a','v','0','1')) {
      ...
      while ((err = MP4TrackReaderGetNextAccessUnit(reader, auH, &auSize, &flags, &cts, &dts)) == MP4NoErr) {
