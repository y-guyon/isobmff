...
bool SampleGroupStrategy::isApplicable(const MetadataMap& items,
...
MP4Err SampleGroupStrategy::inject(const InjectionConfig& config,
    ...
    for (size_t i = 0; i < sortedItems.size(); ++i) {
        ...
        LOG_DEBUG("Processing metadata item {}: frames {}-{} ({} frames)",
                 i + 1, frameStart, frameEnd - 1, item.frame_duration);
        ...
        if (firstSample > videoSampleCount) {
            LOG_WARN("Metadata starts at frame {} but video only has {} samples, skipping",
                    frameStart, videoSampleCount);
            continue;
        }
        ...
        if (firstSample + sampleCount - 1 > videoSampleCount) {
            ...
            LOG_WARN("Metadata extends beyond video track: clamping from {} to {} samples",
        ...
        LOG_INFO("Mapped video samples {}-{} to T.35 group {} ({} bytes)",
                firstSample, firstSample + sampleCount - 1, groupIndex, item.payload.size());
    }
