...
// Parse JSON Folder, convert metadata item to syntax element and return binary data in buffer
MetadataMap parseMetadataFolder(const std::string& metadataFolder, int logLevel) {
  MetadataMap items;
  ...
  for (auto& entry : fs::directory_iterator(metadataFolder)) {
    ...
    if (path.extension() == ".json") {
      ...
      // Get base name for logging/debugging
      auto baseName = path.stem().string();  // e.g. ST2094-50_IMG_0564_metadataItems
      
      std::cout << "Loaded metadata: " << baseName 
                << " -> metadata size=" << payloadBinaryData.size() << " bytes"
                << " || timing frames [" << frame_start << " - " << (frame_start + frame_duration - 1) << "]\n";

      // Create metadata item with binary data in buffer
      // Store original JSON path for reference/debugging
      MetadataItem item{frame_start, frame_duration, path.string(), payloadBinaryData};
...
void decodeBinaryData(const std::string& inputFile, int logLevel) {
                                              ...
                                              std::vector<MetadataItem>& sortedItems)
{
  ...
  for (auto& [start, item] : sorted) {
    ...
    // Get size from payload_data buffer
    u32 payloadSize = static_cast<u32>(item.payload_data.size());
    metadataSizes.push_back(payloadSize);
                                    ...
                                    u32 local_key_id)
{
...

  {
    ...
    for (u32 n = 0; n < sampleCount; ++n)
    {
      ...
      // Use payload_data buffer
      if (item.payload_data.empty()) 
      {
        std::cerr << "Empty payload_data for metadata item at frame " << item.frame_start << "\n";
        err = MP4BadDataErr;
        goto bail;
      }
      
      if (item.payload_data.size() != metadataSizes[n]) 
      {
        std::cerr << "Payload size mismatch for metadata item at frame " << item.frame_start 
                  << ": expected " << metadataSizes[n] << " bytes, got " << item.payload_data.size() << " bytes\n";
        err = MP4BadDataErr;
        goto bail;
      }
      ...
      std::memcpy(dst, item.payload_data.data(), metadataSizes[n]);
      dst += metadataSizes[n];
    }
  }
...
static MP4Err extractMebxSamples(MP4Movie moov, const std::string& inputFile, const std::string& t35PrefixHex, int logLevel)
{
  ...
  for (u32 i = 1; ; ++i) 
  {
    ...
    MP4DisposeHandle(sampleH);
    decodeBinaryData(outFile, logLevel);
  }
...
int main(int argc, char** argv) {
  ...
  // Add debug level option (global, not subcommand-specific)
  // Initialize logger
  int logLevel = 0;  // Default level->LOGLEVEL_ERROR
  app.add_option("-d,--debug-level", logLevel,
                 "Debug/verbose level (0=Off, 1=Error, 2=Warning, 3=Info, 4=Debug, 5=Trace)")
  ->default_val(0)
  ->check(CLI::Range(0, 5));
  ...
  if (*inject) 
  {
    ...
    auto items = parseMetadataFolder(metadataFolder, logLevel);
  ...
  else if (*extract) 
  {
    ...
    if (mode == "mebx") {
      err = extractMebxSamples(moov, inputFile, t35PrefixHex, logLevel);
      if (err) {
