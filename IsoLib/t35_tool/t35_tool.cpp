...
MetadataMap parseMetadataFolder(const std::string& metadataFolder, int logLevel) {
  ...
  for (auto& entry : fs::directory_iterator(metadataFolder)) {
    ...
    if (path.extension() == ".json") {
      ...
      auto binPathGen  = path.parent_path() / (baseName + "_gen.bin");
      // Write the binary data into a file
      std::ofstream outFile(binPathGen, std::ios::out | std::ios::binary);
      if (outFile.is_open()) {
        for (size_t i = 0; i < st2094_50.payloadBinaryData.payload.size(); ++i) {
          outFile.write(reinterpret_cast<const char*>(&st2094_50.payloadBinaryData.payload[i]), sizeof(st2094_50.payloadBinaryData.payload[i]));
        }
      ...
      std::cout << "Loaded metadata: " << baseName << " -> metadata size=" << st2094_50.payloadBinaryData.payload.size() << " bytes || timimg frames [" 
                << frame_start << " - " << (frame_start + frame_duration - 1) << "]\n";

      // Write using the binary data generated by this tool from the json
      MetadataItem item{frame_start, frame_duration, binPathGen.string(), payloadBinaryData};
